...
public class OptionSelector {
    ...
    public static List<OptionMeta> selectATMPlusMinusOne(
        ...
        List<InstrumentInfo> filtered =
                instruments.stream()
                        .filter(i -> i.name.equals(underlying))
        ...
        // 4️⃣ Select options
        List<OptionMeta> result =
                filtered.stream()
                        .filter(i -> i.expiry.equals(nearestExpiry))
                        .filter(i -> strikes.contains(i.strike))
                        .map(i -> new OptionMeta(
                                i.token,
                                futToken,
                                i.type,
                                i.strike,
